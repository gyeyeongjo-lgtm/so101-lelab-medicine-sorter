...
- Jetson의 LeLab 서비스, 설치 소스, calibration, robot record, systemd 설정은 변경하지 않았다.
- Mac에는 진단 문서·메타데이터 전용 검사 스크립트·테스트·Git 저장소만 추가했다.
...
현재는 새 Jetson 패치가 없으므로 실행할 롤백도 없다. 기존 선행 패치의 제거는 정상 캘리브레이션을 깨뜨릴 수 있어 별도 검증 없이 수행하지 않는다.
